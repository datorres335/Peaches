...
@Composable
fun GetLocation(locationUtils: LocationUtils, context: Context, dynamicColors: ColorScheme){
    val requestPermissionLauncher = rememberLauncherForActivityResult(
        ...
        onResult = {
            ...
            if(permissions[Manifest.permission.ACCESS_FINE_LOCATION] == true){
                ...
                // TODO (not sure if we need to implement this section) ???????????????????????????????????????????????
            }
            else {
...
fun GetLocationPreview(){
    GetLocation(locationUtils = LocationUtils(LocalContext.current), context = LocalContext.current, dynamicColors = dynamicDarkColorScheme(LocalContext.current))
}